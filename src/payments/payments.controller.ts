...
export class PaymentsController {
  ...
  async getCardToken(@Req() req) {
    ...
    const cardToken = await this.prisma.cardToken.findUnique({
      where: { companyId },
    });
    if (!cardToken) {
  ...
  async cancelSubscription(@Req() req) {
    ...
    if (!subscription.active)
      throw new BadRequestException('Assinatura já está inativa.');
    if (!subscription.asaasSubscriptionId)
      throw new BadRequestException('ID Asaas não encontrado.');
  ...
  async tokenizeCard(
    ...
    const existing = await this.prisma.cardToken.findUnique({
      where: { companyId },
    });
    if (existing) {
      return {
  ...
  async subscribe(
    @Req() req,
    @Body()
    body: { planId: number; creditCardToken: string; isAnnual: boolean },
  ) {
    const companyId = req.user.companyId;
    if (!companyId) throw new BadRequestException('Empresa não vinculada.');
    ...
    if (!company?.asaasCustomerId)
      throw new BadRequestException('Customer Asaas não criado.');
